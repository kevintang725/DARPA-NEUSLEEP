...
For the first reference about how to map the HCP-MMP1 atlas to the MNI space see this [figshare article](https://figshare.com/articles/HCP-MMP1_0_projected_on_MNI2009a_GM_volumetric_in_NIfTI_format/3501911).
...
At the time of writing this documentation, this corresponds to:
...
First, the atlas was reoriented to match the preferred FSL orientation with the command:

```
$ fslreorient2std MNI_Glasser_HCP_v1.0.nii.gz MNI_Glasser_HCP_v1.0.nii.gz
```

Then the following command was used to get the center of gravity of each label (the -K pre-option allows to treat them as separate sub-masks) and to redirect the output to a .txt file:

```
$ fslstats -K MNI_Glasser_HCP_v1.0.nii.gz MNI_Glasser_HCP_v1.0.nii.gz -C >> HCP-MMP1_COG.txt
```

Finally, the labels from the "Talairach.xml" file were replaced by the ordered list of the HCP-MMP1 labels and by their respective coordinates from the previous command.
