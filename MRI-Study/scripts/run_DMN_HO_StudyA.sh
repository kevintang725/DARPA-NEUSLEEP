...
# - Scans under:  ${ROOT}/sub-*_ses-*_resting_*.results
#   e.g., sub-WANG_NEUSLEEP_STUDYA_001_ses-NEUSLEEP_resting_PostFUS.results
...
#
# Usage:
#   ./run_DMN_HO.sh -root /path/to/base -roidir /path/to/HO/Masks -out /path/to/out
# ======================================================================
...
# Expect folders like:
#   sub-WANG_NEUSLEEP_STUDYA_001_ses-NEUSLEEP_resting_PostFUS.results
foreach d ( $BASE/sub-*_ses-*_resting_*.results )
  if ( ! -d "$d" ) continue
  ...
  # SUBJ: everything before "_ses-..."
  set SUBJ = `echo "$folder" | awk -F'_ses-' '{print $1}'`
  # SES: the ses- token only
  set SES  = `echo "$folder" | sed -E 's/.*(resting_[^_]+).*/\1/'`
