...
time Rscript -e "library(pTFCE);library(oro.nifti);Z=readNIfTI(\"$IN\");MASK=readNIfTI(\"$MASK\");x=read.table(\"smoothness_$BASE.txt\");V=x[2,2];Rd=x[1,2]*x[2,2];write.table(Rd, \"Rd_$BASE.txt\", row.names=F, col.names=F);PTFCE=ptfce(Z, MASK, V=V, Rd=Rd, resels=x[3,2]);writeNIfTI(PTFCE\$Z, \"pTFCE_Z_$BASE\");write.table(PTFCE\$fwer0.05.Z, \"ptfce_zthr.txt\", row.names = F, col.names = F)"
