...
%dev = visadev('USB0::0xF4ED::0xEE3A::SDS1EDEC5R1189::INSTR'); % replace with your actual VISA address
%fwrite(dev, "*IDN?");
%idn = readline(dev);
%disp(idn);
dev = icdevice('your_driver.mdd', 'USB0::0xF4ED::0xEE3A::SDS1EDEC5R1189::INSTR', 'LegacyMode', false);
connect(dev);
...
%% Scan Parameters
dimension_mm = 100;   % in mm
transducer_diameter = 20; % in mm
Increment.X = 0.5;  % in mm
Increment.Y = 0;  % in mm
Increment.Z = 0;  % in mm
...
try
    ...
    for j = 1:ceil(Dim.X)
       ...
       % Pressure
        ...
        % Intensity
        Isppa(j) = PD*1e6*Pn(j)^2/(2*rho*c);
        Ispta(j) = DC*1e6*Pn(j)^2/(2*rho*c);

        display("Scanning: " + X + "," + Y + "," + Z);
        plotdata(tA{i,j,k}, tB{i,j,k}, sA{i,j,k}, sB{i,j,k} , X , Y , Z, P(j,:), PF(j,:), Pn(j), Pp(j), Ispta(j), Isppa(j), dist)
...
%% Plotting
function plotdata(tA, tB, sA, sB , X , Y , Z, P, PF, Pn, Pp, Ispta, Isppa, dist)
    tA = tA*1e6;
    tB = tB*1e6;
